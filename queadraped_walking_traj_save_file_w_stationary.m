...
clc,clear
% Parameters
t_total = 2.5;              % Total time for the cycle (seconds)
...
v = 5;                     % Constant gait speed in m/s
...
if num_steps==1 %for single gait cycle trajectories (making waypoints for optimization code)
    ...
    hip_motion_F=-f1(phi);
    knee_motion_F=-f2(phi); %ignore left as left is just right traj with phase offset
    hip_motion_H=-f4(phi);
    knee_motion_H=-f5(phi); %ignore left as left is just right traj with phase offset
    traj_times=linspace(0,t_total,samples_per_leg );
...
title(['Hip Angles: Walking (', num2str(samples_per_leg), ' waypoints)']);
grid on;
subplot(2,1,2);
...
xlabel('Time (s)'); ylabel('Angle (rad)');
legend('fore left', 'hind left', 'fore right', 'hind right');
title(['Knee Angles: Walking (', num2str(samples_per_leg), ' waypoints)']);
grid on;
